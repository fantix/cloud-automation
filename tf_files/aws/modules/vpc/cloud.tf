...
resource "aws_cloudwatch_log_subscription_filter" "csoc_subscription" {
  ...
  destination_arn = "arn:aws:logs:${data.aws_region.current.name}:${var.csoc_account_id}:destination:${var.subscription_filter}_logs_destination"
  #log_group_name  = "${var.vpc_name}"
  log_group_name  = "${var.subscription_filter}"
  filter_pattern  = ""
}
